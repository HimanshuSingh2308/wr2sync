...
socket.on('SyncOver',function(req){
    ...
    console.info('all traffic is',t, "ms /" , traffic,'b');
    ...
    parseFile(current_file,function(type,data,start,stop){
        ...
        for(i=0; i < numBlocks; i++){
            if(match_table[block_index] >= 0){
                ...
                doc_offset+=4;
                for(var j = 0; j < current_blocksize;j++){
...
export const load_blocks = function (job) {
    ...
    var all_bytelength = 0;
    var doc_offset = 3;
    parseFile(current_file,function(type,data,start,stop){
        var checksumstart = new Date().getTime();
        var checksumdoc = BSync.createChecksumDocument(block_size,data);
        var docView = new Uint32Array(checksumdoc);
        var numBlocks = docView[1];
